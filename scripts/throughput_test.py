...
def update_it_num(root_path, current_it_num):
    ...
    with open(run_file, "w") as f:
        updated_it_num= current_it_num+1
        f.write(str(updated_it_num))  
...
def execute_queries(client_id, root_path, query_path, query_list, scale_factor, it_num):
    ...
    for filename in query_list:
        ...
        try:
            ...
            # pbar.update(1)
            print(f"client_{client_id}: '{filename}' executed successfully on in {execution_time} seconds.")
    ...
    df.to_csv(os.path.join(root_path, f'throughput_test_stats_sf_{scale_factor}_client_{client_id}_it_{it_num}.csv'), index=False)
...
for client_id in range(1, num_clients+1):
    ...
    thread = threading.Thread(target=execute_queries, args=(client_id, root_path, query_path, random_query_list,  scale_factor, current_it))
...
with  open(os.path.join(root_path, f"throughput_test_stats_sf_{scale_factor}.txt"), "a") as f:
        f.write(f"Iteration {current_it}: Total time taken by throughput test= {total_exec_time} seconds \n")
