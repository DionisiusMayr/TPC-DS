...
def execute_maintenance_queries(root_path, query_files_path, scale_factor, it_num):
    ...
    for sql_file in query_files_list:
        with open(os.path.join(query_files_path, sql_file), 'r') as file:
            ...
            df.to_csv(os.path.join(root_path, f'maintenance_test_stats_sf_{scale_factor}_it_{it_num}.csv'), index=False)
...
execute_maintenance_queries(root_path= root_path, query_files_path= maintenance_query_path, scale_factor= scale_factor, it_num= current_it)
t_end= datetime.now()
t_exec= (t_end - t_start).total_seconds()

with open(os.path.join(root_path, f"maintenance_test_stats_sf_{scale_factor}.txt"), "a") as f:
    f.write(f"Iteration {current_it}: total time taken for running maintenace test on scale factor {scale_factor} = {t_exec} seconds\n")
