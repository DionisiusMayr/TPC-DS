...
def copy_into_db(sql_file):
    ...
    try:
        ...
        copy_db_time_list.append([sql_file, start_time, end_time, execution_time])
    ...
    print(f"copy_into_db_stats for scale factor {scale_factor} saved successfully")

# Function to create tables
def create_table(create_table_sql_file):
    conn = pg.connect(**db_params)
    cur = conn.cursor()
...
new_database_name=f"tpcds_sf{scale_factor}"
...
if not database_exists(conn, new_database_name):
    ...
    print(f"database tpcds_sf{scale_factor} created successfully")
...
copy_into_db(data_path)
t_end= datetime.now()
t_exec= (t_end - t_start).total_seconds()
print(f"preprocessing takes {t_exec} seconds")
